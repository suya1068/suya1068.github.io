...
class PopupProductPayment extends Component {
    constructor(props) {
        ...
        this.getPayMethod = this.getPayMethod.bind(this);
        ...
        this.layoutReceipt = this.layoutReceipt.bind(this);
    ...
    onPay() {
        ...
        if (!date || date === "") {
            ...
            API.reservations.reserveToPackage(readyParams).then(response => {
                ...
                if (utils.stringToBoolen(response.success)) {
                    const completeParams = {
                        product_no,
                        pay_uid: response.imp_uid
                    };
                    this.state.receipt = response;
                    return API.reservations.reserveToProductPay(response.merchant_uid, completeParams);
                } else if (response && response.error_msg) {
                    let payMessage;
                    try {
                        payMessage = response.error_msg.replace(/[+]+/gi, " ").split(" | ")[1];
                    } catch (error) {
                        payMessage = "결제가 취소되었습니다";
                    }
                    PopModal.alert(payMessage, {
                        callBack: () => {
                            if (product_no && !isNaN(product_no)) {
                                location.replace(`/products/${product_no}`);
                            }
                        }
                    });
                } else {
                ...
                if (response && response.status === 200) {
                    ...
                    tracking.conversion();
                    //console.log("popupProductPayment", response, this.state);

                    //utils.ad.fbqEvent("Purchase", { value: prop.reserve.total_price, currency: "KRW" });
                    //window.fbq("track", "Purchase", { value: prop.reserve.total_price, currency: "KRW" });
                    // this.wcsEvent(prop.reserve.total_price);
                    //utils.ad.wcsEvent("1", prop.reserve.total_price);
    ...
    getPayMethod(type) {
        ...
        const item = method.find(m => (m.value === type));
    ...
    layoutPayment() {
        ...
        return (
            <div className="layout__body" onScroll={this.onScroll}>
                                ...
                                <div className="layer__border">
                                    <div className="layer__column">
                                        ...
                                        <div className="layer__row auto__flex align__center caption__content padding__half">
                                            ...
                                            <span>촬영일이 3개월 이후인 예약 건은 임의 날짜로 예약 후 고객센터로 변경할 날짜를 알려주세요!</span>
                                        </div>
                                    </div>
                                </div>
                                {packageList.map((p, i) => {
                                                ...
                                                {p.optionList.map(o => {
                                                    ...
                                                    return (
                                                        <div key={`option-select-${o.code}`} className="products__options__checkbox__item">
                                                            <div>
                                                                <div className="title">
                                                                    <Checkbox checked={o.selected} type="yellow_small" resultFunc={code => this.onSelectOption(o.code, code)}>{o.title}</Checkbox>
                                                                    {content ?
                                ...
                                <div className="layer__border">
                                    <div className="layer__column text-right">
                                        <div className="layer__row padding__default">
                                            ...
                                            <div>
                                                {name || <Input className="text-right" type="text" value={user_name} onChange={(e, n, v) => this.setState({ user_name: v })} max="8" />}
                                            </div>
                                        </div>
                                        <div className="hr" />
                                        <div className="layer__row padding__default">
                                        ...
                                        <div className="layer__row padding__default">
                                            ...
                                            <div>
                                                {email || <Input className="text-right" type="email" value={user_email} onChange={(e, n, v) => this.setState({ user_email: v })} max="50" />}
                                            </div>
                                        </div>
                                        <div className="hr" />
                                        <div className="layer__row padding__default">
                                    ...
                                    <div className="layer__column padding__default">
                                        ...
                                        <div className="text__content">
                                            <div className="layer__row">
                                                {method.map(m => {
                                                    return (
                                                        <button key={`pay-method-${m.value}`} className={classNames("f__button f__button__theme__select__pink", m.checked ? "active" : "")} onClick={() => this.checkMethod(m)}>{m.name}</button>
                                                    );
                                                })}
                                            </div>
                                        </div>
                ...
                <div className="layout__body__side__right">
                    <div className="layer" ref={ref => (this.aside = ref)}>
                        <div className="layer__body">
                            <div className="layer__container">
                                <div className="layer__column">
                                ...
                                <div className="layer__border overflow__hidden">
                                    <div className="layer__column">
                                        <div className="layer__row padding__default">
                                            ...
                                            <div className="text-right">
                                                {date ? mewtime(mewtime.strToDate(date)).format("YYYY년 M월 D일") : "촬영날짜를 선택해주세요"}
                                            </div>
                                        </div>
                                        <div className="hr" />
                                        <div className="layer__row padding__default">
                                            ...
                                            {totalPrice === null ?
                                                <div className="text-right">패키지를 선택해주세요</div> :
                                                <div className="text-right">{utils.format.price(totalPrice)}<span>원</span></div>
                                            }
                                        </div>
                                        <div className="hr" />
                                        <div className="layer__row">
                                            <button className="f__button f__button__theme__yellow padding__default" onClick={this.onPay}>결제하기</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
    ...
    layoutReceipt() {
        const { receipt, reserve, user_name, user_phone, user_email } = this.state;
        const { product_no } = this.props.data;
                            ...
                            <div className="layer__container">
                                <div className="layer__border">
                                    <div className="layer__column padding__default align__center">
                                        ...
                                        <div className="text__content">
                                            주문번호 : {utils.format.formatByNo(receipt.merchant_uid)}
                                        </div>
                                    </div>
                                </div>
                                {isSuccess && reserve ?
                                    <div className="layer__container">
                                        <div className="layer__border">
                                            <div className="layer__column text-right">
                                                <div className="layer__row padding__default">
                                                    ...
                                                    <div>
                                                        {mewtime(reserve.reserve_dt).format("YYYY년 MM월 DD일")}
                                                    </div>
                                                </div>
                                                <div className="hr" />
                                                <div className="layer__row padding__default">
                                                ...
                                                <div className="layer__row padding__default">
                                                    ...
                                                    <div>
                                                        {utils.format.price(reserve.price)}원
                                                    </div>
                                                </div>
                                                <div className="hr" />
                                                <div className="layer__row padding__default">
                                                    ...
                                                    <div>
                                                        {utils.format.price(reserve.add_price)}원
                                                    </div>
                                                </div>
                                                <div className="hr" />
                                                <div className="layer__row padding__default">
                                                    ...
                                                    <div style={{ color: "#ff326c" }}>
                                                        {utils.format.price(reserve.total_price)}원
                                                    </div>
                                                </div>
                                                <div className="hr" />
                                                <div className="layer__row padding__default">
                                                    ...
                                                    <div>
                                                        {reserve.pay_type === "vbank"
                                                            ...
                                                            : this.getPayMethod(reserve.pay_type)
                                                        }
                                                    </div>
                                                </div>
                                            </div>
                                        ...
                                        <div className="layer__border">
                                            <div className="layer__column text-right">
                                                ...
                                                {reserve.custom_data && reserve.custom_data.user_msg ? [
                                                    <div className="hr" />,
                                                    <div className="layer__row padding__default">
                                                        <h2 className="text__header">
                                                            요청사항
                                ...
                                <div className="layer__row padding__default nth__margin__left">
                                    <a className="f__button" href="/">메인페이지가기</a>
                                    {isSuccess && reserve ?
                                        <a className="f__button" href={`/users/progress/${reserve.pay_type === "vbank" ? "ready" : "payment"}`}>예약확인하기</a>
                                        : <a className="f__button" href={`/products/${product_no}`}>상품페이지가기</a>
                                    }
                                </div>
                            </div>
                        </div>
    ...
    render() {
        const { receipt, reserve } = this.state;
                ...
                <div className="layout">
                    <div className="layout__header">
                        <div className="products__payment__breadcrumb">
                            <div className={classNames("payment__breadcrumb__process", reserve ? "" : "active")}>
                                <i className={`m-icon m-icon-won-${reserve ? "gray" : "sky"}`} />
                                <div className="title">
                                    예약 결제하기
                                </div>
                            </div>
                            ...
                            <div className={classNames("payment__breadcrumb__complete", reserve ? "active" : "")}>
                                <i className={`m-icon m-icon-complete-${reserve ? "sky" : "gray"}`} />
                                <div className="title">
                                    주문완료
                                </div>
                            </div>
                    ...
                    {reserve ? this.layoutReceipt() : this.layoutPayment()}
                </div>
            </div>
        );
    }
