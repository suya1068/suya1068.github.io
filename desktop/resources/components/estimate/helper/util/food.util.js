...
const calculatePrice = (form, priceInfo) => {
    ...
    if (form[PROPERTYS.DIRECTING_NUMBER.CODE]) {
        if (form[PROPERTYS.LOCATION.CODE] === PLACE_PROPERTY.STUDIO.CODE) {
            totalPrice += (form[PROPERTYS.DIRECTING_NUMBER.CODE] * priceInfo.directing_price_studio);
            totalPrice *= 1;
        } else if (form[PROPERTYS.LOCATION.CODE] === PLACE_PROPERTY.OUTSIDE.CODE) {
            totalPrice += (form[PROPERTYS.DIRECTING_NUMBER.CODE] * priceInfo.directing_price_outside);
            totalPrice *= 1;
        }
    }

    if (form[PROPERTYS.PLACE.CODE]) {
        totalPrice += priceInfo.outside_add_price;
        totalPrice *= 1;
        hasAlphas = true;
    } else {
        hasAlphas = false;
 ...
 * @param form
 // * @param priceInfo
 */
const exchangeResultText = (key, form) => {
    let resultText = "";
    ...
    if (key === STEP_KEY.FIRST && (form[PROPERTYS.NUMBER.CODE] || form[PROPERTYS.DIRECTING_NUMBER.CODE])) {
        if (form[PROPERTYS.SHOT_KIND.CODE] === SHOT_KIND_PROPERTY.NUKKI_SHOT.CODE) {
            resultText = `누끼 ${form[PROPERTYS.NUMBER.CODE] || 0} 컷 (컷당 20,000원)`;
        } else if (form[PROPERTYS.SHOT_KIND.CODE] === SHOT_KIND_PROPERTY.DIRECTING_SHOT.CODE && form[PROPERTYS.DIRECTING_NUMBER.CODE]) {
            resultText = `연출 ${form[PROPERTYS.DIRECTING_NUMBER.CODE] || 0} 컷 (컷당 50,000원)`;
            if (form[PROPERTYS.LOCATION.CODE] === PLACE_PROPERTY.STUDIO.CODE) {
                resultText = `연출 ${form[PROPERTYS.DIRECTING_NUMBER.CODE]} 컷 (컷당 60,000원)`;
            }
        } else if (form[PROPERTYS.SHOT_KIND.CODE] === SHOT_KIND_PROPERTY.N_PLUS_D_SHOT.CODE) {
            resultText = `누끼 ${form[PROPERTYS.NUMBER.CODE] || 0} 컷 (컷당 20,000원) / 연출 ${form[PROPERTYS.DIRECTING_NUMBER.CODE] || 0} 컷 (컷당 50,000원)`;

            if (form[PROPERTYS.LOCATION.CODE] === PLACE_PROPERTY.STUDIO.CODE && form[PROPERTYS.DIRECTING_NUMBER.CODE]) {
                resultText = `누끼 ${form[PROPERTYS.NUMBER.CODE] || 0} 컷 (컷당 20,000원) / 연출 ${form[PROPERTYS.DIRECTING_NUMBER.CODE]} 컷 (컷당 60,000원)`;
            }
        }
    }

    if (key === STEP_KEY.SECOND && !!form[PROPERTYS.LOCATION.CODE]) {
        if (form[PROPERTYS.LOCATION.CODE] === PLACE_PROPERTY.STUDIO.CODE) {
            resultText = "스튜디오 내방";
        } else if (form[PROPERTYS.LOCATION.CODE] === PLACE_PROPERTY.OUTSIDE.CODE) {
            resultText = "출장";
    ...
    if (key === STEP_KEY.THIRD && form[PROPERTYS.NOTE.CODE]) {
        resultText = `${form[PROPERTYS.NOTE.CODE]}`;
    }
...
const onRadio = (form, code) => {
    if (code === PLACE_PROPERTY.STUDIO.CODE) {
        form[PROPERTYS.LOCATION.CODE] = code;
        form[PROPERTYS.PLACE.CODE] = "";
    } else if (code === PLACE_PROPERTY.OUTSIDE.CODE) {
        form[PROPERTYS.LOCATION.CODE] = code;
        form[PROPERTYS.PLACE.CODE] = "";
    } else if (code === PLACE_PROPERTY.SEOUL.CODE) {
        form[PROPERTYS.PLACE.CODE] = code;
    } else if (code === PLACE_PROPERTY.ETC.CODE) {
        form[PROPERTYS.PLACE.CODE] = code;
    }
    ...
    if (st === 1) {
        ...
        if (form.note) {
            if (form.location === "studio") {
                data[step].ACTIVE = true;
            } else if (form.location === "outside" && form.place) {
                data[step].ACTIVE = true;
            }
        }
    }
