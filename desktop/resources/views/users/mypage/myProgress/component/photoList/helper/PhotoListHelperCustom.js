...
export default class PhotoListOrigin extends PhotoListHelper {
    ...
    getPhotoList(buy_no, product_no, user_type) {
        return API.reservations.reservePhotosCustom(buy_no, product_no, user_type, this.fix_offset, this.getVarLimit())
            .then(response => {
    ...
    getMorePhotoList(buy_no, product_no, user_type) {
        return API.reservations.reservePhotosCustom(buy_no, product_no, user_type, this.getVarOffset(), this.fix_limit)
            .then(response => {
                const data = response.data;
                this.setCounts({
