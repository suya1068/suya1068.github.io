...
class MainRecommend extends Component {
    ...
    gaEvent(product_no) {
        utils.ad.gaEvent("개인_메인", "메인 추천상품 선택", `상품번호 = ${product_no}`);
            ...
            <div className="ma__rcmd">
                <div className="rcmd__tab">
                    {data.map((o, i) => {
                        ...
                        return (
                            <div key={o.code} className={classNames("tab__item", { active })} onClick={() => this.onClickIndex(i)}>
                                {o.name}
                            </div>
                        );
                    })}
                    ...
                    {item && item.list && Array.isArray(item.list) && item.list.length ?
                        <div className="rcmd__swiper">
                            <div className="swiper-wrapper">
                                {item.list.map(o => {
                                    let url = `${__SERVER__.thumb}/normal/crop/243x243/${o.thumb_img}`;
                                    if (!o.thumb_img || o.thumb_img === constant.DEFAULT_IMAGES.BACKGROUND) {
                                        url = `${__SERVER__.img}/${constant.DEFAULT_IMAGES.BACKGROUND}`;
                                    }
                                    return (
                                        <a key={`item_${o.product_no}`} className="swiper-slide" href={`/products/${o.product_no}`} target="_blank" onClick={() => this.gaEvent(o.product_no)}>
                            ...
                            {item.list.length > 4 ? [
                                <div key="btn_prev" className="rcmd__sld__ar prev"><i className="_icon__arrow__ml action" /></div>,
                                <div key="btn_next" className="rcmd__sld__ar next"><i className="_icon__arrow__mr action" /></div>] : null
                            }
                        </div> : null
                    }
                </div>
