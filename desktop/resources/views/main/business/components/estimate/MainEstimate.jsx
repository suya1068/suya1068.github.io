...
export default class MainEstimate extends Component {
    ...
    onConsultSearchArtist(estimate_no = "") {
        this.gaEvent("오픈견적");
        this.onMoveChargeList(estimate_no);
    }

    onMoveChargeList(estimate_no) {
        const { currentCategory } = this.state;
        // mcl === move charge list
        window.open(`/products?category=${currentCategory}&mcl=1`);
        // location.href = `/products?category=${currentCategory}&mcl=1`;
    }

    render() {
        const { currentCategory, access_type, device_type } = this.state;
        return (
            <section className="main__estimate">
                <h2 className="main-section__title">
                    정직한 촬영비용, 3초만에 확인해보세요!
                </h2>
                <div className="main__estimate__box">
                    <div className="container">
                        <VirtualEstimateTab onSelectTab={this.onSelectTab}>
                            {/*<VirtualEstimate*/}
