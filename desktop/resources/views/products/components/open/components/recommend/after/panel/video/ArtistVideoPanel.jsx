...
export default class ArtistVideoPanel extends Component {
    ...
    createPortfolio(idx, obj) {
        ...
        if (Object.prototype.hasOwnProperty.call(obj, "portfolio_video") && portfolio_video.list.length > 0) {
            pv.map((video, i) => {
                content.push(
                    <div className="portfolio__thumb-video" style={{ cursor: "pointer" }} key={`artist__portfolio__${idx}__${i}`} onClick={() => this.onMovePage(obj, link, "추천_포폴")}>
                                ...
                                <div className="swiper-slide" key={`pre-recommend-artist__${idx}`}>
                                    ...
                                    <div className="pre-recommend-artist__content__box__info-artist">
                                        ...
                                        <div className="inquire-btn">
                                            ...
                                            {/*<button className="_button inquire" onClick={() => this.onConsultArtist(item)}>{`${item.nick_name} 작가에게 문의하기`}</button>*/}
                                        </div>
                                    </div>
                                </div>
                            );
