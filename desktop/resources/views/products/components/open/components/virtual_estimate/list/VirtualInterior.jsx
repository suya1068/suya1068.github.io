    ...
    composeActiveList(list, p_code, c_code) {
        return list.reduce((result, obj) => {
            obj.reduce((r, o) => {
                ...
                // disabled 변경 로직 (상세에서 필요)
                if (p_code === "init") {
                    if (o.CODE === "is_exterior" || o.CODE === "inside_cut_compose") {
                    ...
                    if (o.CODE === "place") {
                        helper.targetReduce(ob => helper.changeProp(["seoul"], ob, true, "DEFAULT"), o);
                        helper.targetReduce(ob => helper.changeProp(["gyeonggi"], ob, false, "DEFAULT"), o);
                        helper.targetReduce(ob => helper.changeProp(["etc"], ob, false, "DEFAULT"), o);
                    }
                }

                // 익스테리어 필요 컷수 활성화 여부
                if (p_code === PROPERTYS.IS_EXTERIOR.CODE && c_code === HAS_PROPERTY.NEED.CODE) {
                    helper.changeProp([PROPERTYS.EXTERIOR_NUMBER.CODE], o, false);
                } else if (p_code === PROPERTYS.IS_EXTERIOR.CODE && c_code === HAS_PROPERTY.NEEDLESS.CODE) {
                    helper.changeProp([PROPERTYS.EXTERIOR_NUMBER.CODE], o, true);
                }

                // 내부 합성 필요 컷수 활성화 여부
                if (p_code === PROPERTYS.INSIDE_CUT_COMPOSE.CODE && c_code === HAS_PROPERTY.NEED.CODE) {
                    helper.changeProp([PROPERTYS.INSIDE_COMPOSE_NUMBER.CODE], o, false);
                } else if (p_code === PROPERTYS.INSIDE_CUT_COMPOSE.CODE && c_code === HAS_PROPERTY.NEEDLESS.CODE) {
                    helper.changeProp([PROPERTYS.INSIDE_COMPOSE_NUMBER.CODE], o, true);
                }
