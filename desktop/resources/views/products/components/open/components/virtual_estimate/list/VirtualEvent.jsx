...
export default class VirtualFood extends Component {
    constructor(props) {
        ...
        this.setReceiveData = this.setReceiveData.bind(this);
     ...
     * @returns {*}
     */
    composeActiveList(list, p_code, c_code, data) {
        return list.reduce((result, obj) => {
            obj.reduce((r, o) => {
                ...
                if (p_code === "shot_kind") {
                    if (o.CODE === "place") {
                        targetReduce(ob => changeProp(["seoul"], ob, true, "DEFAULT"), o);
                        targetReduce(ob => changeProp(["gyeonggi", "etc"], ob, false, "DEFAULT"), o);
                    }
                }

                // 사진만 선택 시 촬영시간, 단체사진, 단체사진필요컷수 활성화
                if (c_code === "photo_only") {
                    changeProp(["is_all_shot"], o, false);
                    changeProp(["video_directing", "all_shot_need_number", "video_directing_time", "subscribe"], o, true);
                    if (o.CODE === "video_directing" || o.CODE === "subscribe") {
                        targetReduce(ob => changeProp(["needless", "need"], ob, false, "DEFAULT"), o);
                    }

                    if (o.CODE === "is_all_shot") {
                        targetReduce(ob => changeProp(["needless"], ob, true, "DEFAULT"), o);
                        targetReduce(ob => changeProp(["need"], ob, false, "DEFAULT"), o);
                    }
                }

                // 영상만 선택 시 촬영시간, 영상편집, 자막 활성화
                if (c_code === "video_only") {
                    changeProp(["is_all_shot", "all_shot_need_number", "video_directing_time"], o, true);
                    changeProp(["video_directing", "subscribe"], o, false);
                    if (o.CODE === "is_all_shot") {
                ...
                if (c_code === "video_together") {
                    changeProp(["is_all_shot", "video_directing", "subscribe"], o, false);
                    changeProp(["all_shot_need_number", "video_directing_time"], o, true);

                    if (o.CODE === "video_directing" || o.CODE === "subscribe" || o.CODE === "is_all_shot") {
                        targetReduce(ob => changeProp(["needless"], ob, true, "DEFAULT"), o);
                        targetReduce(ob => changeProp(["need"], ob, false, "DEFAULT"), o);
                    }
                }
                ...
                if (p_code === "is_all_shot") {
                    if (c_code === "need") changeProp(["all_shot_need_number"], o, false);
                    if (c_code === "needless") changeProp(["all_shot_need_number"], o, true);
                }
                // 영상 편집 필요 선택 시 편집 영상 시간 활성화
                if (p_code === "video_directing") {
                    if (c_code === "need") changeProp(["video_directing_time"], o, false);
                    if (c_code === "needless") changeProp(["video_directing_time"], o, true);
                }
                r.push(o);
                return r;
            }, []);
     ...
     * @param p_code
     * @param c_code
     * @param data 부모에게서 조회된 견적정보
     */
    onActive(e, p_code, c_code, data) {
        const { list, price_info, form, alphas, error, total_price } = this.state;
        const _list = this.composeActiveList(list, p_code, c_code, data);
    ...
    onChange(value, code) {
        ...
        // if (code === PROPERTYS.ALL_SHOT_NEED_NUMBER.CODE) {
        //     error.all_shot_need_number.show = !value || false;
        //
        //     if (form.shot_kind === "video_together") {
        //     }
        // }
        //
        // if (code === PROPERTYS.VIDEO_DIRECTING_TIME.CODE) {
        //     error.video_directing_time.show = !value || false;
        //
        //     if (form.shot_kind === "video_together") {
        //     }
        // }
    ...
    render() {
        const { list, form, total_price, prev_total_price, alphas, numbers, error, isAlphas } = this.state;
