...
export default class PortfolioList extends Component {
    ...
    render() {
        ...
        return (
            <section className="product_list__portfolio-list portfolio-list product__dist" id="portfolio_list">
                <div className="portfolio-list__head">
                    <h2 className="section-title">전체 포트폴리오</h2>
                </div>
                <div className="portfolio-list__tags__list">
                    <div className="portfolio-list__tags__list-wrap">
                        <div className={classNames("tag-item", "selected")} onClick={() => this.onRecommend("")} ref={node => (this.allTag = node)}>전체</div>
                        {products.category_tag.map((tag, idx) => {
                            ...
                            return (
                                <div className={classNames("tag-item", className)} key={`tag__${idx}`} onClick={() => this.onRecommend(tag, !!className)}>{tag}</div>
                            );
                        })}
                    </div>
                </div>
                ...
                <div className="product-alert">
                    결과물은 포스냅 홈페이지 및 홍보용으로 게시될 수 있습니다.<br />
                    게시를 원치 않는 경우 미리 말씀해주시면 노출되지 않습니다.
                </div>
            </section>
        );
    }
