...
export default class VirtualProduct extends Component {
    ...
    composeActionList(list, p_code, c_code) {
        ...
        return list.reduce((result, obj) => {
            // 활성화 변경 로직 =============
            if (obj.CODE === p_code) {
                helper.columnDefaultListRender(obj.PROP, c_code);
            }
            ...
            if (c_code === SHOT_KIND_PROPERTY.NUKKI_SHOT.CODE) {
                ...
                if (obj.CODE === PROPERTYS.DIRECTING_KIND.CODE) {
                    helper.targetReduce(ob => helper.changeProp([need, needless], ob, false, "DEFAULT"), obj);
                }
                if (obj.CODE === PROPERTYS.DIRECTING_KIND.CODE) {
                    helper.targetReduce(ob => helper.changeProp([DIRECTING_PROPERTY.BASIC.CODE, DIRECTING_PROPERTY.PROXY.CODE], ob, false, "DEFAULT"), obj);
                }
                if (obj.CODE === PROPERTYS.PROXY_DIRECTING_KIND.CODE) {
                    helper.targetReduce(ob => helper.changeProp([PROXY_DIRECTING_PROPERTY.SHOT_PROXY.CODE, PROXY_DIRECTING_PROPERTY.DIRECTING_SHOT_PROXY.CODE], ob, false, "DEFAULT"), obj);
                }
                if (obj.CODE === PROPERTYS.SIZE.CODE) {
                    helper.targetReduce(ob => helper.changeProp([SIZE_PROPERTY.SMALL.CODE, SIZE_PROPERTY.LARGE.CODE], ob, false, "DEFAULT"), obj);
                    helper.targetReduce(ob => helper.changeProp([SIZE_PROPERTY.SMALL.CODE], ob, true, "DEFAULT"), obj);
                }
                if (obj.CODE === PROPERTYS.MATERIAL.CODE) {
                    helper.targetReduce(ob => helper.changeProp([MATERIAL_PROPERTY.GLOSSLESS.CODE, MATERIAL_PROPERTY.GLOSS.CODE], ob, false, "DEFAULT"), obj);
                    helper.targetReduce(ob => helper.changeProp([MATERIAL_PROPERTY.GLOSSLESS.CODE], ob, true, "DEFAULT"), obj);
                }
                ...
                if (obj.CODE === PROPERTYS.DIRECTING_KIND.CODE) {
                    helper.targetReduce(ob => helper.changeProp([DIRECTING_PROPERTY.BASIC.CODE, DIRECTING_PROPERTY.PROXY.CODE], ob, false, "DEFAULT"), obj);
                    helper.targetReduce(ob => helper.changeProp([DIRECTING_PROPERTY.BASIC.CODE], ob, true, "DEFAULT"), obj);
                }
                ...
                if (obj.CODE === PROPERTYS.MATERIAL.CODE) {
                    helper.targetReduce(ob => helper.changeProp([MATERIAL_PROPERTY.GLOSS.CODE, MATERIAL_PROPERTY.GLOSSLESS.CODE], ob, false, "DEFAULT"), obj);
                }
            ...
            if (p_code === PROPERTYS.DIRECTING_KIND.CODE && c_code === DIRECTING_PROPERTY.PROXY.CODE) {
                ...
                if (obj.CODE === PROPERTYS.PROXY_DIRECTING_KIND.CODE) {
                    helper.targetReduce(ob => helper.changeProp([PROXY_DIRECTING_PROPERTY.SHOT_PROXY.CODE, PROXY_DIRECTING_PROPERTY.DIRECTING_SHOT_PROXY.CODE], ob, false, "DEFAULT"), obj);
                    helper.targetReduce(ob => helper.changeProp([PROXY_DIRECTING_PROPERTY.SHOT_PROXY.CODE], ob, true, "DEFAULT"), obj);
                }
            }
    ...
    onActive(e, p_code, c_code) {
        ...
        this.setState({
            ...
            // console.log("price_info", this.state.price_info);
            this.calculatePrice();
        });
    }
    ...
    render() {
        const { list, total_price, prev_total_price, form, alphas, error, numbers, info, isAlphas } = this.state;
        ...
        return (
            <div className={detailPage ? "detailPage-content-box" : ""}>
                <VirtualEstimateView {...flags} list={list} info={info} form={form} error={error} numbers={numbers} onActive={this.onActive} onChange={this.onChange} />
