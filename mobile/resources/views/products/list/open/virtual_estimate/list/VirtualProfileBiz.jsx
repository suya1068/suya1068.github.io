...
export default class VirtualProfileBiz extends Component {
    ...
    composeActiveList(list, p_code, c_code) {
        return list.reduce((result, obj) => {
            ...
            if (p_code === "init") {
                ...
                if (obj.CODE === "place") {
                    helper.targetReduce(ob => helper.changeProp(["studio"], ob, true, "DEFAULT"), obj);
                    helper.targetReduce(ob => helper.changeProp(["outside_s"], ob, false, "DEFAULT"), obj);
                    helper.targetReduce(ob => helper.changeProp(["outside_e"], ob, false, "DEFAULT"), obj);
                }
            }

            if (p_code === PROPERTYS.IS_ALL_SHOT.CODE && c_code === HAS_PROPERTY.NEED.CODE) {
                helper.changeProp([PROPERTYS.ALL_SHOT_NEED_NUMBER.CODE], obj, false);
            } else if (p_code === PROPERTYS.IS_ALL_SHOT.CODE && c_code === HAS_PROPERTY.NEEDLESS.CODE) {
                helper.changeProp([PROPERTYS.ALL_SHOT_NEED_NUMBER.CODE], obj, true);
            }
            result.push(obj);
            return result;
        }, []);
    ...
    onActive(e, p_code, c_code) {
        ...
        // ===== 알파값 설정 =====
        if (p_code === "place") {
            alphas[p_code] = c_code === "outside_e" || false;
        }
    ...
    render() {
        const { list, total_price, prev_total_price, form, alphas, numbers, error, isAlphas } = this.state;
