...
export default class VirtualFood extends Component {
    ...
    composeActiveList(list, p_code, c_code) {
        return list.reduce((result, obj) => {
            ...
            if (c_code === SHOT_KIND_PROPERTY.NUKKI_SHOT.CODE) {
                helper.changeProp([PROPERTYS.NUKKI_PRODUCT_NUMBER.CODE], obj, false);
                helper.changeProp([PROPERTYS.DIRECTING_PRODUCT_NUMBER.CODE], obj, true);
            }

            if (c_code === SHOT_KIND_PROPERTY.DIRECTING_SHOT.CODE) {
                helper.changeProp([PROPERTYS.NUKKI_PRODUCT_NUMBER.CODE], obj, true);
                helper.changeProp([PROPERTYS.DIRECTING_PRODUCT_NUMBER.CODE], obj, false);
            }
            ...
            if (c_code === PLACE_PROPERTY.STUDIO.CODE) {
                helper.changeProp([PROPERTYS.PLACE.CODE], obj, true);
                if (obj.CODE === PROPERTYS.PLACE.CODE) {
                    helper.targetReduce(ob => helper.changeProp([PLACE_PROPERTY.SEOUL.CODE, PLACE_PROPERTY.ETC.CODE], ob, false, "DEFAULT"), obj);
                }
            }

            if (c_code === PLACE_PROPERTY.OUTSIDE.CODE) {
                helper.changeProp([PROPERTYS.PLACE.CODE], obj, false);
                if (obj.CODE === PROPERTYS.PLACE.CODE) {
                    helper.targetReduce(ob => helper.changeProp([PLACE_PROPERTY.SEOUL.CODE, PLACE_PROPERTY.ETC.CODE], ob, false, "DEFAULT"), obj);
                    helper.targetReduce(ob => helper.changeProp([PLACE_PROPERTY.SEOUL.CODE], ob, true, "DEFAULT"), obj);
                }
            }
    ...
    onActive(e, p_code, c_code) {
        ...
        // ===== 에러값 설정 =====
        // if (c_code === SHOT_KIND_PROPERTY.NUKKI_SHOT.CODE) {
        //     error[PROPERTYS.NUKKI_PRODUCT_NUMBER.CODE].show = true;
        //     error[PROPERTYS.DIRECTING_PRODUCT_NUMBER.CODE].show = false;
        // }
        //
        // if (c_code === SHOT_KIND_PROPERTY.DIRECTING_SHOT.CODE) {
        //     error[PROPERTYS.NUKKI_PRODUCT_NUMBER.CODE].show = false;
        //     error[PROPERTYS.DIRECTING_PRODUCT_NUMBER.CODE].show = true;
        // }
        //
        // if (c_code === SHOT_KIND_PROPERTY.N_PLUS_D_SHOT.CODE) {
        //     error[PROPERTYS.NUKKI_PRODUCT_NUMBER.CODE].show = true;
        ...
        // // ===== 알파값 설정 =====
        // if (p_code === PROPERTYS.PLACE.CODE) {
        //     alphas[p_code] = c_code === PLACE_PROPERTY.ETC.CODE || false;
        // }
        // // ===== 끝./ 알파값 설정 =====
        //
        // // ===== 에러값 설정 =====
        ...
        //
        // if (c_code === PLACE_PROPERTY.OUTSIDE.CODE) {
        //     form[PROPERTYS.PLACE.CODE] = c_code;
        // }
        // ===== 끝./ 에러값 설정 =====
