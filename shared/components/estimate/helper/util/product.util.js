...
const calculatePrice = (form, priceInfo) => {
    ...
    const sizeSmall = form[PROPERTYS.SIZE.CODE] === SIZE_PROPERTY.SMALL.CODE;
    const materialGlossless = form[PROPERTYS.MATERIAL.CODE] === MATERIAL_PROPERTY.GLOSSLESS.CODE;

    if (form[PROPERTYS.NUMBER.CODE]) {
        if (sizeSmall && materialGlossless) {
            totalPrice += (form[PROPERTYS.NUMBER.CODE] * priceInfo.nukki_price_1);
            totalPrice *= 1;
        } else if ((sizeSmall && !materialGlossless) || (!sizeSmall && materialGlossless)) {
            totalPrice += (form[PROPERTYS.NUMBER.CODE] * priceInfo.nukki_price_2);
            totalPrice *= 1;
        } else if (!sizeSmall && !materialGlossless) {
            totalPrice += (form[PROPERTYS.NUMBER.CODE] * priceInfo.nukki_price_4);
            totalPrice *= 1;
        }
    }

    if (form[PROPERTYS.DIRECTING_NUMBER.CODE] && form[PROPERTYS.DIRECTING_KIND.CODE] !== DIRECTING_PROPERTY.PROXY.CODE) {
        totalPrice += form.directing_number * priceInfo.directing_price;
        totalPrice *= 1;
    }
 ...
 * @param device_type
 */
const exchangeResultText = (key, form, priceInfo, device_type) => {
    let resultText = "";
    const sizeCode = PROPERTYS.SIZE.CODE;
    const materialCode = PROPERTYS.MATERIAL.CODE;
    const sizeSmall = SIZE_PROPERTY.SMALL.CODE;
    const sizeLarge = SIZE_PROPERTY.LARGE.CODE;
    const mateGloss = MATERIAL_PROPERTY.GLOSS.CODE;
    const mateGlossless = MATERIAL_PROPERTY.GLOSSLESS.CODE;
    ...
    if (key === STEP_KEY.FIRST && (form[PROPERTYS.NUMBER.CODE] || form[PROPERTYS.DIRECTING_NUMBER.CODE])) {
        let nukkiCutPrice = 5000;
        if (form[sizeCode] === sizeSmall && form[materialCode] === mateGlossless) {
            nukkiCutPrice = priceInfo.nukki_price_1;
        } else if ((form[sizeCode] === sizeSmall && form[materialCode] === mateGloss) || (form[sizeCode] === sizeLarge && form[materialCode] === mateGlossless)) {
            nukkiCutPrice = priceInfo.nukki_price_2;
        } else if (form[sizeCode] === sizeLarge && form.material === mateGloss) {
            nukkiCutPrice = priceInfo.nukki_price_4;
        }

        if (form[PROPERTYS.SHOT_KIND.CODE] === SHOT_KIND_PROPERTY.NUKKI_SHOT.CODE) {
            resultText = `누끼 ${form.number || 0} 컷 (컷당 ${utils.format.price(nukkiCutPrice)}원)`;
        } else if (form[PROPERTYS.SHOT_KIND.CODE] === SHOT_KIND_PROPERTY.DIRECTING_SHOT.CODE) {
            resultText = `연출 ${form.directing_number || 0} 컷 (컷당 40,000원)`;
            if (form[PROPERTYS.DIRECTING_KIND.CODE] === DIRECTING_PROPERTY.PROXY.CODE && form[PROPERTYS.PROXY_NUMBER.CODE]) {
                resultText = `촬영대행 ${form[PROPERTYS.PROXY_NUMBER.CODE] || 0} 제품 (제품당 100,000원)`;
            }
        } else if (form[PROPERTYS.SHOT_KIND.CODE] === SHOT_KIND_PROPERTY.N_PLUS_D_SHOT.CODE) {
            resultText = `누끼 ${form[PROPERTYS.NUMBER.CODE] || 0} 컷 (컷당 ${utils.format.price(nukkiCutPrice)}원) / 연츨 ${form[PROPERTYS.DIRECTING_NUMBER.CODE] || 0} 컷 (컷당 40,000원)`;

            if (device_type === "mobile") {
                resultText = `누끼 ${form[PROPERTYS.NUMBER.CODE] || 0} 컷 (컷당 ${utils.format.price(nukkiCutPrice)}원)\n연츨 ${form[PROPERTYS.DIRECTING_NUMBER.CODE] || 0} 컷 (컷당 40,000원)`;
            }

            if (form[PROPERTYS.DIRECTING_KIND.CODE] === DIRECTING_PROPERTY.PROXY.CODE && form[PROPERTYS.PROXY_NUMBER.CODE]) {
                resultText = `누끼 ${form[PROPERTYS.NUMBER.CODE] || 0} 컷 (컷당 ${utils.format.price(nukkiCutPrice)}원) / 촬영대행 ${form[PROPERTYS.PROXY_NUMBER.CODE] || 0} 제품 (제품당 100,000원)`;
                if (device_type === "mobile") {
                    resultText = `누끼 ${form[PROPERTYS.NUMBER.CODE] || 0} 컷 (컷당 ${utils.format.price(nukkiCutPrice)}원)\n촬영대행 ${form[PROPERTYS.PROXY_NUMBER.CODE] || 0} 제품 (제품당 100,000원)`;
                }
            }
        }
    }

    if (key === STEP_KEY.SECOND && (form[sizeCode] || form[materialCode])) {
        const size = form[sizeCode] === sizeSmall ? "소형제품" : "대형제품";
        const material = form[materialCode] === mateGloss ? "유광제품" : "무광제품";
        resultText = `${material} / ${size}`;
    }

    if (key === STEP_KEY.THIRD && form[PROPERTYS.NOTE.CODE]) {
