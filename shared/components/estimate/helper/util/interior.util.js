...
const exchangeResultText = (key, form, priceInfo) => {
    ...
    if (key === STEP_KEY.THIRD) {
        ...
        if (isExterior && !isComposeCut) {
            resultText = `익스테리어 필요 ${form[PROPERTYS.EXTERIOR_NUMBER.CODE]} 컷 (컷당 50,000)\n내부 컷 합성 불필요`;
        } else if (!isExterior && isComposeCut) {
            resultText = `익스테리어 불필요\n내부 컷 합성 필요 ${form[PROPERTYS.INSIDE_COMPOSE_NUMBER.CODE]} 컷 (컷당 40,000)`;
        } else if (isExterior && isComposeCut) {
            resultText = `익스테리어 필요 ${form[PROPERTYS.EXTERIOR_NUMBER.CODE]} 컷 (컷당 50,000)\n내부 컷 합성 필요 ${form[PROPERTYS.INSIDE_COMPOSE_NUMBER.CODE]} 컷 (컷당 40,000원)`;
        } else {
            resultText = "익스테리어 불필요\n내부 컷 합성 불필요";
        }
    }
