...
export default class LastStep extends Component {
    ...
    renderStep(form) {
        ...
        if (this.props.category === "FOOD") {
            switch (form.location) {
                ...
                case "outside": content = this.renderDirecting("FOOD"); break;
                default:
            }
    ...
    renderInterior() {
        const { data, device_type } = this.props;
        const { form } = this.state;
    ...
    renderProfileBiz() {
        ...
        return (
            <div className="last-step food">
                <div className="info-item-box" style={{ border: "1px solid #dedede", padding: 20, backgroundColor: "#fff" }}>
                    {data.PROP.map((obj, idx) => {
                        return (
                            ...
                            >
                                <div className="head-title-wrap">
                                    <p className="head-title" style={{ fontSize: 15, fontWeight: "bold", marginBottom: 15 }}>{utils.linebreak(obj.NAME)}</p>
                                    <div
                                        className=""
                                        style={{
                                            display: "flex",
    ...
    profileInputArea(o) {
        ...
        return (
            <div className="footer-input__box" style={{ marginTop: 0, marginLeft: 10 }}>
    ...
    renderInputSide(obj, value) {
        ...
        return (
            <div className="footer-input__box" style={{ width: "100%" }}>
                {unit && unit.PRE &&
    ...
    renderDirectingFood(imageProp, freeProp) {
        const { form } = this.state;
        return (
            <div className="last-step third-directing">
                <div className="info-item-box">
                    <div className="info-item third-nukki" style={{ marginBottom: 5 }}>
                        <p>{freeProp.NAME}</p>
                ...
                <div className="info-item-box">
                    {imageProp.map((obj, idx) => {
                        ...
                        return (
                            <div className={classNames("info-item", { "col": !isImage })} key={`information__item__${obj.CODE}_${idx}`}>
                                <div className={classNames("info-item__head")} style={{ borderBottom: "none" }}>
                                    <div className="head-title-wrap" style={{ flexDirection: "column", width: "100%", alignItems: "flex-start" }}>
                                        <p className="head-title" style={{ marginBottom: 15 }}>{obj.NAME}</p>
                                        <div className="" style={{ display: "flex", width: "100%" }}>
                                            {obj.PROP.map((o, i) => {
                                                ...
                                                const selectType = optionType === "select";
                                                return (
                                                    <div key={`food_${i}`} style={{ display: "flex", width: "50%" }}>
                                                        {this.renderBoxType(radioType, selectType, o)}
    ...
    renderDirectingDefault(imageProp, freeProp) {
        const { device_type, data } = this.props;
        const { form } = this.state;
        ...
        return (
            <div className="third-directing">
                ...
                <div className="info-item third-nukki" style={{ marginTop: 5 }}>
                    <p>{freeProp.NAME}</p>
                    {this.renderInputSide(freeProp, form.note)}
                </div>
                }
    ...
    onRadio(e, code) {
        ...
        const { estimateUtils } = this.props;
        const { form } = this.state;
        this.props.onRadio(e, code);
        this.props.onChangeForm({ ...estimateUtils.onRadio(form, code) });
        // const { estimateUtils } = this.props;
        // const { form } = this.state;
        //
        // this.setState({
