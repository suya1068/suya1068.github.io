...
class PaymentOffer extends Component {
                ...
                .then(params => {
                    if (params) {
                        payment.payment(params).then(result => {
                            ...
                            if (result) {
                                if (result.success) {
                                    API.reservations.reserveToProductPay(result.merchant_uid, { pay_uid: result.imp_uid }).then(response => {
                                        ...
                                        const errorMessage = error.data ? error.data : error.error_msg;
                                        Modal.show({
                            ...
                            Modal.show({
                                type: MODAL_TYPE.ALERT,
                                content: utils.linebreak("요청서가 미노출이거나\n견적서를 이미 결재하셨습니다.")
                            });
                        });
                    } else {
                        Modal.close(MODAL_TYPE.PROGRESS);
                        Modal.show({
                            type: MODAL_TYPE.ALERT,
                            content: utils.linebreak("결제 준비중 오류가 발생했습니다.\n잠시 후 다시 시도해주세요.\n지속적인 오류시 고객센터로 문의해주세요.")
                        });
                    }
                })
                .catch(error => {
    ...
    render() {
        ...
        if (receipt) {
            ...
            if (reserve.pay_type === "vbank") {
                methodContent = utils.linebreak(`입금계좌 : ${reserve.vbank_name}\n계좌번호 : ${reserve.vbank_num}\n입금기한 : ${reserve.vbank_date}`);
            } else {
                const objMethod = method.find(m => {
                    return m.value === reserve.pay_type;
                });
            ...
            return (
                <div className={classNames("products__payment__mobile", { is__mobile: isMobile })}>
                    ...
                    <div className="products__payment__content payment__process">
                        <div className="products__payment__status">
                            ...
                            {!isSuccess ?
                                <div className="message">{receipt.error_msg || "결제가 취소되었습니다"}</div>
                                : null
                            }
                        </div>
                        {isSuccess && reserve ?
                        ...
                        <div className="products__payment__button">
                            <a className="f__button f__button__theme__fill-black" href="/">메인페이지가기</a>
                            {isSuccess && reserve ?
                                <a className="f__button" href={`/users/progress/${reserve.pay_type === "vbank" ? "ready" : "payment"}`} target="_blank">예약확인하기</a>
                                : null
                            }
                        </div>
                    </div>
                                        ...
                                        {option.map((o, i) => {
                                            ...
                                            return (
                                                <tr key={`option_${i}`}>
                                                    ...
                                                    <td>{utils.format.price(o.count)}</td>
                                                    <td>{utils.format.price(o.total_price)}원</td>
                                                </tr>
                                            );
                                        })}
                                    </tbody>
...
PaymentOffer.popTypes = {};
PaymentOffer.defaultProps = {};
