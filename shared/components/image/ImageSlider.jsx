...
class ImageSlider extends Component {
    constructor(props) {
        ...
        this.initTouchPosition = this.initTouchPosition.bind(this);
    }
    ...
    componentWillReceiveProps(nextProps) {
        if (this.props.children !== nextProps.children) {
            this.state.renderSlider = nextProps.children;
            // this.setState({
            //     renderSlider: nextProps.children
            // });
        }
    }
    ...
    onTouchStart(e) {
        const touchPosition = this.state.touchPosition;
        if (e.type === "touchstart" && e.touches.length === 1) {
            touchPosition.touch_start_x = e.touches[0].pageX;
        }
    }
    ...
    onTouchMove(e) {
        ...
        if (e.type === "touchmove" && e.touches.length === 1) {
            drag_dist = e.touches[0].pageX - touchPosition.touch_start_x;

            this.setState({
                touchPosition: update(touchPosition, { move_dx: { $set: drag_dist } })
            });
        }
    }
    ...
    onTouchEnd(e) {
        ...
        const maxIndex = this.state.renderSlider.length;
        if (e.type === "touchend" && e.touches.length === 0) {
            if (touchPosition.move_dx > 55 && data.index > 0) {
                this.moveIndex(data.index - 1);
            } else if (touchPosition.move_dx < -55 && data.index < maxIndex - 1) {
                this.moveIndex(data.index + 1);
            }
        }
        this.initTouchPosition();
    }
    ...
    initData() {
        ...
        // 네비게이션 초기화
        if (data.nav) {
            if (!data.nav.type || data.nav.type === "") {
                data.nav.type = "circle";
            } else if (["circle", "number"].indexOf(data.nav.type) === -1) {
                data.nav.type = "circle";
            }
        ...
        if (list) {
            if (this.props.children) {
                ...
                this.state.renderSlider = list.map((obj, i) => {
                    ...
                    return (
                        <li key={`image-list-${i}`}>
                            <Img image={obj} isCrop={data.crop} isImageCrop={data.imageCrop} isImageResize={data.imageResize} isContentResize={data.contentResize} />
                        </li>
                    );
                });
            ...
            if (data.nav) {
                ...
                this.state.renderNav = list.map((obj, i) => {
                    return (
                        <li key={`image-nav-${i}`} className="nav-dotted" onClick={e => this.moveIndex(i)}>&nbsp;</li>
                    );
                });
            }
        }
    ...
    calculLayoutArrow() {
        ...
        if (arrow) {
            ...
            this.state.renderArrow = [
                <div key="slider-arrow-left" className="slider-arrow-left" onClick={() => this.moveIndex(data.index - 1)} style={style.left}>
                    &lt;
                </div>,
                <div key="slider-arrow-right" className="slider-arrow-right" onClick={() => this.moveIndex(data.index + 1)} style={style.right}>
                    &gt;
                </div>
            ];
        }
    ...
    render() {
        ...
        return (
            <div className="image-slider-box" key="image-slider" onMouseEnter={this.onMouseEnter} onMouseLeave={this.onMouseLeave} ref={ref => { this.sliderbox = ref; }}>
                <div
...
ImageSlider.propTypes = {
    ...
    data: PropTypes.shape({
        ...
        nav: PropTypes.shape({
            type: PropTypes.string,  // circle, number
            position: PropTypes.string,
            inout: PropTypes.string,
            posX: PropTypes.oneOfType([PropTypes.number, PropTypes.string]),
            posY: PropTypes.oneOfType([PropTypes.number, PropTypes.string])
